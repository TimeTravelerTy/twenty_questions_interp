...
**Current milestone:** **M5b DONE — answer-rollout patch at 27B end_ready L14-L18 confirms the M5 null and strengthens the improvisation headline.** Job 7779768: 1024 patched trials, 98.3% kept target, **2/870** off-diagonal flip-to-source, **3/3480** answer slots flipped (0.086%), baseline rollouts fully deterministic (0/32). Even under the answer-rollout protocol — where the model regenerates A_1..A_4 under the patched end_ready state before the reveal — the residual at end_ready L14-L18 fails to bias the model's own yes/no answer head, ruling out the "teacher-forced (Q,A) history hides the patch effect" confound that could have explained the M5 null. One isolated tiger→shark/attempt_431 trial flipped both answers (3/4) and reveal, but the neighborhood is otherwise rock-solid: only 1 of 5 tiger sources moves attempt_431, all other 6 source classes leave it at shark, and the other shark target (attempt_592) is 0/32 fragile. Not auto-escalating; the narrow follow-up is widening the tiger-source pool into attempt_431 (cheap, ~20 trials). Writeup: `docs/progress/M5b-patch-answer-rollout-27b-endready.md`.

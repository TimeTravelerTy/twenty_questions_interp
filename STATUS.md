...
**Current milestone:** **M5b in flight — answer-rollout patch at 27B end_ready (job 7779618, L14-18).** Fixes a gap in the M5 patch sweep: prior runs teacher-forced the manifest's (Q,A) history as text, so the model could re-derive target class from visible answers. New `--answer-rollout` flag in `scripts/patch_anchor.py` re-generates the model's A_1..A_4 under the patched end_ready state before reveal. Two metrics: answer-flip-delta vs baseline rollout, and reveal-flip-to-source. If both null, the M5 "improvisation is scale-robust" headline strengthens; if answer-flips delta > 0, end_ready steers answer behavior even if not the reveal. Writeup skeleton: `docs/progress/M5b-patch-answer-rollout-27b-endready.md`. Escalation chain if null: L12-20 → L1-62 full → multi-anchor end_ready+end_model_q1..q4.
...
**Last agent:** Claude
**Last updated:** 2026-05-27 (M5b: added `--answer-rollout` to `scripts/patch_anchor.py`; submitted job 7779618 on gpu_1 — 27B end_ready L14-18 with per-turn answer re-decode under the patched state. Result pending; design doc at `docs/progress/M5b-patch-answer-rollout-27b-endready.md`.)
**Prior agent:** Claude

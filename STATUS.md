...
**Current milestone:** **M5b structurally complete — L16 steering with rollout is decisively null, L58 steering is positive but speaks to readout-assembly not early commitment. The improvisation story is locked.** New job 7783019 (L16 steering at end_ready with `--answer-rollout` + `scope=from_anchor`, alphas {1,3,10}): 97.9% kept-target at α=1 and α=3 with **0 flip-to-src** and **0 answer-flip slots** at both, 91.1% kept at α=10 with 1.0% flip-to-src and only 3/768 answer slots flipped (all in one trial: gorilla→shark/attempt_431 → reveal=tiger, the same `[T,F,F,T]` boundary-fragility pattern as the L35-L45 multi-anchor patch). Baselines 32/32 fully deterministic. This is the *strongest* possible "L16 direction is epiphenomenal" test — direct injection at the probe peak, scoped to positions ≥ end_ready, with rollout giving 5 forward passes to bias the model. The model regenerates the same answers 99.6% of the time. Reframed L58 result: **not tautological** — the direction has to survive 4 more transformer blocks of attention+MLP to drive output. Rather, complementary: L58 (pre_reveal_gen) is the **readout-assembly position** where the model has just finished re-deriving the class via attention; steering there flips because it's where commitment exists. The L16 null + L58 positive is unified by the improvisation thesis: model re-derives class at each pass, summary-but-not-substrate intermediate, finished commitment in the late residual. Per-class probe analysis (6/7 classes, shark missing) shows **horse 0.59, tiger 0.57** as top per-class LR — exactly the L58 clean-flip responders. Predicted-most-steerable pair `tiger ↔ gorilla` (score 5.11) is the only L16 cell that showed monotonic α-dependence in either rollout or non-rollout steering. Heuristic validated. Writeups updated. Ready for handoff — next session should consider position-bisection (steering at pre_answer_qN with rollout) to localize the commitment-formation transition.
...
**Last agent:** Claude
**Last updated:** 2026-05-28 (M5b complete + ready for handoff. Added `--answer-rollout` to `steer_class_direction.py`, ran L16 steering with rollout + from_anchor scope (job 7783019): 0% flip-to-src and 0 answer flips at α=1 and α=3, 1% flip-to-src and 3/768 answer slots at α=10 (single attempt_431 tiger-attractor trial). Decisive null. Reframed L58 result in writeup — not tautological (4 layers of attention+MLP between L58 and unembed); it's complementary evidence that commitment-as-readout exists late while commitment-as-substrate at end_ready does not. **Open: position-bisection** — steering with rollout at pre_answer_qN anchors (LR peaks L38) and end_model_q4 to localize the transition from "no commitment" (end_ready) to "load-bearing commitment" (pre_reveal_gen). This is the next rigorous step and would tighten the blog story. Also open: per-class probe accuracy analysis at L16/L58 (local run still in progress on partial v2 captures, 6/7 classes — shark missing because alphabetical scp didn't reach attempt_431/592 before being killed). Files of note for handoff: docs/progress/M5b-steering-27b-endready-L16.md (now has L16-rollout section + reframed L58 + "what this is / isn't evidence for"), scripts/steer_class_direction.py (rollout + ablate modes), scripts/analyze_class_directions.py (per-class probe geometry).)
**Prior agent:** Claude
